function [errorparams, logmlh0, bic]=mlhrateeiglingen3c1Ex_Gerror(initparams,LUbounds,frburstdata,cumindex,indexone,fixed, stateid,exid,chans,denatConc,drvintv,drvpnt,maxThread,GPU_load)
...
if ~exist('maxThread','var') 
    maxThread=13;  %set to 13
end

if ~exist('GPU_load','var')
    GPU_load=0.1;  %set to 0.1
end
...
twoColorId=find(stateid~=1); %1: 3color, 2:DA1, 3:DA2
for ii=1:numel(twoColorId)
    onet3rColor=frburstdata(cumindex(twoColorId(ii))+1:cumindex(twoColorId(ii)+1),end);
    onet3rColor(onet3rColor==1)=2;
    frburstdata(cumindex(twoColorId(ii))+1:cumindex(twoColorId(ii)+1),end)=onet3rColor;
end
...
logmlh=mlhrateeiglingen3c1Ex_CU(initparammat,LUboundsNew,frburstdata,cumindex,indexone,cntrate,fixedMat*0,stateid,exid,denatConc,maxThread,GPU_load);
